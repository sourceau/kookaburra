/* Copyright (C) RENDA - All Rights Reserved
 * Unauthorized copying of this file, via any medium is strictly prohibited
 * Proprietary and confidential
 * R150.0
 */

// Importing libraries from the import map
...
// Importing TWEEN using its URL directly as it's not in the import map
...
//
//****************
initModel("scene", true, window.postTitle);
...
function downloadScene(privacy, fileType) {
  ...
  var file_name = window.postTitle;
  }
...
//////////////////
 ...
 const canvasContainer = document.getElementById('canvasContainer');
        ...
        const rightBtnGroup = document.getElementById('rightBtnGroup');
        const snapshotBtn = document.getElementById('snapshotBtn');
        const openModalBtn = document.querySelector('.open-modal'); // Select Btn 5
...
sceneBackgroundBtn.addEventListener("click", function () {
  toggleDrawer("open");
  SetUI("scene-background");
});
sceneLightingBtn.addEventListener("click", function () {
  toggleDrawer("open");
  SetUI("scene-lighting");
});
sceneEffectsBtn.addEventListener("click", function () {
  toggleDrawer("open");
  SetUI("scene-effects");
});
importPlaneBtn.addEventListener("click", function () {
  toggleDrawer("open");
  SetUI("import-plane");
});
...
importDigitalObjectsBtn.addEventListener("click", function () {
   toggleDrawer("open");
   SetUI("import-digital-objects");
});
importPrintObjectsBtn.addEventListener("click", function () {
   toggleDrawer("open");
   SetUI("import-print-objects");
});
importPackagingObjectsBtn.addEventListener("click", function () {
   toggleDrawer("open");
   SetUI("import-packaging-objects");
});
importAdvertisingObjectsBtn.addEventListener("click", function () {
   toggleDrawer("open");
   SetUI("import-advertising-objects");
});
...
function toggleHint(text,opacity) {
  ...
  //  $("#hint-text").html(text);
        ...
        function toggleSidePanel(action) {
            if (action === "open") {
                btnGroup.classList.add('hide');
                rightBtnGroup.classList.add('hide');
                snapshotBtn.classList.add('hide');
                sidePanel.classList.add('open');
				modalBackground.classList.add('open'); // Show modal background
                canvasContainer.classList.add('pointer-events-active');
            } else if (action === "close") {
                btnGroup.classList.remove('hide');
                rightBtnGroup.classList.remove('hide');
                snapshotBtn.classList.remove('hide');
                sidePanel.classList.remove('open');
				modalBackground.classList.add('close'); // Show modal background
                canvasContainer.classList.remove('pointer-events-active');
            }
        }
        ...
        function closeModal() {
            ...
            modalBackground.classList.remove('open'); // Hide modal background
        }
  ...
  // Event listeners for side panel
   ...
   // JavaScript for handling accordion functionality
                ...
                var innerButtons = document.querySelectorAll('.accordion .content .sidebar-button');

                innerButtons.forEach(function (innerButton) {
...
//PALETTE BUTTONS
 // Variables to control the number of buttons in each category
  const numGreyscaleButtons = 10;
  const numRandomButtons = 40;
  const numTintButtons = 30;
  const numShadeButtons = 30;
  ...
  // Create buttons based on the defined numbers
  const buttonContainer = document.getElementById('palette-container');
  for (let i = 0; i < numGreyscaleButtons + numRandomButtons + numTintButtons + numShadeButtons; i++) {
    const button = document.createElement('div');
    button.className = 'palette-button';
    paletteContainer.appendChild(button);
    
    // Set initial colors
    if (i < numGreyscaleButtons) {
      // Greyscale buttons: white to black gradient
      let grayValue = Math.floor(i * (255 / (numGreyscaleButtons - 1)));
      button.style.backgroundColor = `rgb(${grayValue},${grayValue},${grayValue})`;
    } else if (i < numGreyscaleButtons + numRandomButtons) {
      // Random color buttons
      button.style.backgroundColor = getRandomColor();
    } else {
      // Tint and shade buttons: initially white
      button.style.backgroundColor = '#ffffff';
    }
    
    // Add click event listener
    button.addEventListener('click', function() {
      // Check if the clicked button is a tint or shade button
      const buttonIndex = Array.from(buttonContainer.children).indexOf(button);
      if (buttonIndex >= numGreyscaleButtons + numRandomButtons && buttonIndex < numGreyscaleButtons + numRandomButtons + numTintButtons + numShadeButtons) {
        return; // Do nothing if it's a tint or shade button
      }
      
      // Reset color of last (numTintButtons + numShadeButtons) buttons to white
      const allButtons = document.querySelectorAll('.palette-button');
      const startTintIndex = numGreyscaleButtons + numRandomButtons;
      
      // Get the clicked button's color
      const clickedColor = this.style.backgroundColor;
      const colorRGB = getRGBColor(clickedColor); // Get RGB values of clicked color
      
      // Tint buttons: buttons from startTintIndex to (startTintIndex + numTintButtons - 1)
      for (let k = 0; k < numTintButtons; k++) {
        let tintRatio = (numTintButtons - k) / (numTintButtons + 1); // Tint ratio (light to dark)
        let tintR = Math.min(Math.floor(colorRGB.r + (255 - colorRGB.r) * tintRatio), 255);
        let tintG = Math.min(Math.floor(colorRGB.g + (255 - colorRGB.g) * tintRatio), 255);
        let tintB = Math.min(Math.floor(colorRGB.b + (255 - colorRGB.b) * tintRatio), 255);
        allButtons[startTintIndex + k].style.backgroundColor = `rgb(${tintR},${tintG},${tintB})`;
      }
      
      // Shade buttons: buttons from (startTintIndex + numTintButtons) to end
      for (let k = 0; k < numShadeButtons; k++) {
        let shadeRatio = (numShadeButtons - k) / numShadeButtons; // Shade ratio (dark to light)
        let shadeR = Math.floor(colorRGB.r * shadeRatio);
        let shadeG = Math.floor(colorRGB.g * shadeRatio);
        let shadeB = Math.floor(colorRGB.b * shadeRatio);
        allButtons[startTintIndex + numTintButtons + k].style.backgroundColor = `rgb(${shadeR},${shadeG},${shadeB})`;
      }
    });
  }
  ...
  function getRGBColor(colorString) {
    let rgb = colorString.substring(4, colorString.length-1)
                         .replace(/ /g, '')
                         .split(',');
    return {
      r: parseInt(rgb[0]),
