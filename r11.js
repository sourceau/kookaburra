...
function initModel(initMode, fileHosted, fileReference) {
 ...
 // }
  ...
  var loader = new GLTFLoader(manager).load("https://s3.ca-central-1.amazonaws.com/renda.warehouse.ca/scenes/r1/renda/1134167.renda", function (glb) {
    scene.add(glb.scene);
...
function SetUI(mode) {
  // Define all container IDs and their corresponding modes
  const containers = {
    "face": "face-container",
    "scene-background": "scene-background-container",
    "scene-lighting": "scene-lighting-container",
    "scene-effects": "scene-effects-container",
    "import-plane": "import-plane-container",
    "save-file": "save-file-container",
    "snapshot": "snapshot-container",
    "import-print-objects": "import-print-objects-container",
    "import-digital-objects": "import-digital-objects-container",
    "import-packaging-objects": "import-packaging-objects-container",
    "import-advertising-objects": "import-advertising-objects-container"
  };

  // Hide all containers by default
  for (const containerId in containers) {
    document.getElementById(containers[containerId]).style.display = "none";
  }

  // Show the container based on the mode
  if (containers.hasOwnProperty(mode)) {
...
function toggleLoadingScreen(visible) {
    ...
    if (visible === false) {
        fadeElementOut(loadingScreen);
    } else {
        fadeElementIn(loadingScreen);
    }
    
    // Optional: Call any additional functions after fading
    onWindowResize();
}
